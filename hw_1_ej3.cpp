...
/*Para la implementacion de la lista, uso como unique pointer head y como raw pointer tail.
Uso move() para mover el nodo y transferir la propiedad de unique pointer del nodo a otro
...
struct lista{
    ...
    nodo*tail;
    size_t size;
    //inicializo la lista con head y tail como punteros a null y con tamanio de la lista=0
...
int push_front(lista *lista_ejemplo, unique_ptr<nodo> nuevo_nodo){
    ...
    Sino, primero pongo al siguiente de nuevo_nodo como head y al head como nuevo_nodo
    */
...
int push_back(lista *lista_ejemplo, unique_ptr<nodo> nuevo_nodo){
    /*Si la lista es 0 mismo caso que el anterior. Sino primero pongo al siguiente 
    del tail como nuevo_nodo y el tail ahora lo apunto a nuevo_nodo
    */
    if (lista_ejemplo->size==0){
        lista_ejemplo->head = move(nuevo_nodo);
        lista_ejemplo->tail = lista_ejemplo->head.get();
...
int insert(lista* lista_ejemplo, unique_ptr<nodo> nuevo_nodo, long unsigned int pos_lista){
    /*Si la pos_lista es 0, pongo al siguiente del nuevo nodo como head y el head lo apunto a nuevo_nodo.
    Si ademas de eso el size de la lista es 0, apunto tail a head.
    ...
    y al siguiente del actual como nuevo nodo. Si me paso del largo de la lista,
...
int erase(lista* lista_ejemplo, long unsigned int pos_lista){
    /*Si la pos_inicial es 0, muevo mi head al siguiente de head, lo que haria que el head inicial, 
    que quedo sin puntero, al terminar la funcion se haga free automaticamente ya que estoy usando smart pointers.
    ...
    itero hasta que pase uno de los dos.
    Si me paso del largo de la lista, apunto el siguiente del actual a null y apunto el tail a mi nodo actual.
    Si no pasa esto, simplemente apunto el siguiente del actual al actual->next->next
    En los dos casos, haria que el nodo que quiera eliminar quede resferenciado,
...
int print_list(lista *lista_ejemplo){
    /*Imprimo la lista usando un puntero auxiliar que recorre desde el head
    hasta que llegue al tail, y en cada iteracion devuelvo el valor del puntero auxiliar
    para luego pasar al siguiente del acutal
    */
    cout<<"Lista completa: \n";
    nodo* actual = lista_ejemplo->head.get();
